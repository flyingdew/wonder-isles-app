...
- [ ] 应用阶段：干扰项来源改为同场景其它字，避免"离得太远"
- [ ] 单字通关：场景中对应物件（河岸太阳/月亮…）从灰变亮的仪式感动画

## W2 · 20 字内容 + 场景小诗

- [ ] 山林 / 村落 / 田野 三个场景全部接入 5 字
- [ ] 每场景通关触发"小诗关"（5 字组诗，拖拽入位）
- [ ] Boss 关：20 字组长诗
- [ ] 音效（毛笔沙沙 / 水声 / 鸟鸣）与 BGM（古琴/箫氛围乐）

## W3 · 系统与家长端

- [x] 本地存档：点亮字 id + 每字回访次数（已在 `ProgressStore` 中实现）
- [ ] 家长视图：点亮总数、最喜欢的字、共读小片段建议
- [ ] 主菜单设置：音量、TTS 开关、清空进度
- [ ] 简单成就：首次点亮、集齐一岛、连续 3 天访问
