...
Release 版本自动接 Sentry 上报 Dart 异常、Flutter framework 错误、原生 crash。核心特性：

- **儿童合规优先**：`sendDefaultPii=false`，不上传截图 / view hierarchy / 定位 / 设备指纹，只上报"发生了什么错"本身。
- **仅 release build 启用**：`kDebugMode` / `kProfileMode` 下 `ErrorReporter.guard()` 与其它 API 均是 no-op，本地开发不会产生噪音、也不会污染线上项目。
- **DSN 通过 `--dart-define=SENTRY_DSN=...` 注入**：源码里不写 DSN，未配置 secret 时 CI 出的包也不会启用 Sentry，行为安全降级。
- **采样**：错误 100%，性能追踪 10%。

### 配置 SENTRY_DSN

到仓库 [Settings → Secrets and variables → Actions](https://github.com/flyingdew/wonder-isles-app/settings/secrets/actions)：

1. 新建 secret：`SENTRY_DSN`
2. 值填 Sentry 项目的 DSN，形如 `https://xxxxxxxx@o123456.ingest.sentry.io/789012`
3. 下一次 tag push（`v*`）出的 APK 与下一次 `main` push 出的 Web 都会自动带上 Sentry

### 校验通路是否打通

在 release 包里：家长视图 → 关于卡片右下角"上报测试"按钮（**只有 `SENTRY_DSN` 已注入时才显示**），点一次就会在 Sentry 里出现一条 `wonder-isles test exception` 事件。
